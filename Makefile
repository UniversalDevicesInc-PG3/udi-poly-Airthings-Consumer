# udi-poly-Airthings-Consumer — XML validation and PG3 release artifacts (tag + per-track zips).
#
...
#                           Then in PG3 UI, edit the plugin and set Version to that exact VERSION.
#   4. `make production`  — push HEAD to the `production` branch (reference) and build
#                           $(NAME)-Production-Standard-<VERSION>.zip.
# The track-specific zip files are the actual deliverables uploaded to PG3.

PYTHON ?= python3
NAME = Airthings-Consumer
GIT_REMOTE ?= origin
BRANCH_BETA ?= beta
...
help:
	...
	@echo "  make production          Push HEAD -> $(GIT_REMOTE)/$(BRANCH_PRODUCTION) and build $(NAME)-Production-Standard-\$$VERSION.zip"
...
clean:
	$(PYTHON) -c "import pathlib, shutil; r = pathlib.Path('.'); [shutil.rmtree(p, ignore_errors=True) for p in r.rglob('__pycache__') if p.is_dir()]"
	rm -f $(NAME)*.zip
...
# Push current HEAD to $(GIT_REMOTE)/$(BRANCH_PRODUCTION) (reference) and build
# $(NAME)-Production-Standard-<VERSION>.zip for upload to PG3.
# Requires clean tree; not detached HEAD.
production:
	@set -e; \
	ROOT=$$(pwd); \
	VERSION=$$(sed -n 's/^VERSION = "\([^"]*\)"$$/\1/p' "$$ROOT/nodes/__init__.py"); \
...
# Run from this directory, or: make -C /path/to/udi-poly-Airthings-Consumer release
